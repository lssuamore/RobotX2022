...
bool func = false;                           // if convert = false, this means according to the current task status, conversion shouldn't be done
...
// THIS func: Obtains the USV NED position
// ACCEPTS: 
// RETURNS: 
// =============================================================================
...
// THIS func: Converts from the relative USV frame to spherical ECEF
// ACCEPTS: 
// RETURNS: 
// =============================================================================
...
// THIS func: Finds the best focal length of the camera sensors, based off a LUT
...
// THIS func: Determines the initial lateral gain to multiply the first lateral disparity calculation by, based off a LUT
...
// THIS func: Determines the final lateral gain to multiply the lateral disparity calculation by, based off a LUT (lat_scale_LUT)
...
void cameraCallBack(const sensor_msgs::ImageConstPtr& camera_msg)
{
	...
	try 
	{
		...
		//using a median filter build in func apply it to each mask with a kernal size of 3 to get rid of small noise
...
{
	if ((msg->name == "perception") && ((msg->state == "ready") || (msg->state == "running")))
	{
		func = true;
		ROS_INFO("PERCEPTION ON");
	}
	else
	{
		func = false;
		ROS_INFO("PERCEPTION OFF");
	}
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber nedpos_sub = nh3.subscribe("usv_ned", 100, NED_Position); // subscribes to NED position
	
	task3_pub = nh2.advertise<geographic_msgs::GeoPoseStamped>("/vrx/perception/landmark", 100); 
