...
//  Author(s) [email]: Hannah Brood, Xavier Vicent, Aaron Osmond, and Matthew Bowen
//  Revisor(s) [Revision Date]:
//  Organization/Institution:  Lake Superior State University RobotX Team AMORE
// 
// ...............................About path_planner.cpp......................................
//  This code acts as the client for sending a heartbeat message through a TCP socket via Ethernet
//
...
#include "jetson/motorStatus_msg.h"
...
// RETURNS: (VOID)
// =============================================================================
void Task_Control (const std_msgs::Int64::ConstPtr& msg)									//Task name subscriber
{
	task = msg->data;
}//END OF //Task_Control()
...
// RETURNS: (VOID)
// =============================================================================
void AMS_status (const jetson::motorStatus_msg::ConstPtr& msg_Status)
{
    status = msg_Status->motorNode;
...
//======================VISION PROJECT==================================================
// THIS FUNCTION: Gets what dock we are docking
// ACCEPTS: std_msgs::String
// RETURNS: (VOID)
// =============================================================================
void Dock_color(const std_msgs::String::ConstPtr& msg)
{
    Color_Dock= msg->data;
    if (Color_Dock == "red")
    {
...
//======================VISION PROJECT==================================================
// THIS FUNCTION: Gets the AMS status for the docking task
// ACCEPTS: std_msgs::String
// RETURNS: (VOID)
// =============================================================================

void Dock_status(const std_msgs::String::ConstPtr& msg)
{
    Status_Dock= msg->data;
    if (Status_Dock == "Docking")
        dockingStatusStr[0] = '1';
    else if (Status_Dock== "Complete")
        dockingStatusStr[0] = '2';
 ...
 * 10/17/22
 */
int heartbeat_sock::sendstr(const char *buff)
{
    char packet[MAX];
...
/*--------------------------------------ORIGINAL---------------
 ...
 //-----------------------------------------------*/
    send(sockfd, packet, strnlen(packet,MAX),0);
...
//---------------------------------------------VISION PROJECT TOPICS-------------------------------------------------------------------------------------------------------
    ros::Subscriber Task_info_sub = n.subscribe("/MC_state", 1,Task_Control);							//Obtains current AMS task
    ros::Subscriber Dock_color_sub = n.subscribe("/Det_color", 1, Dock_color);
    ros::Subscriber Dock_status_sub = n.subscribe("/Det_status", 1, Dock_status);
...
//--------------------------------------------END OF VISION PROJECT----------------------------------------------------------------------------------------------------------
  ...
  //Initialize Socket
    ...
    setsockopt(sock.sockfd,SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive ));
    ...
    sock.servaddr.sin_addr.s_addr = inet_addr(ip);
