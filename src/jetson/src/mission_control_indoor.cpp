...
{
	if ((loop_count > 10)  &&  (navigation_array_initialized) && (propulsion_system_initialized))
	{
...
// RETURNS: (VOID)
// =============================================================================
void state_update(const jetson::task_info::ConstPtr& msg)
{
...
void current_goal_pose_publish()
{
	...
	current_goal_pose_msg.position.x = x_goal;						// sets x-location
	current_goal_pose_msg.position.y = y_goal;						// sets y-location
	current_goal_pose_msg.position.z = 0.0;										// sets z-location
	current_goal_pose_msg.psi.data = psi_goal;						// sets psi
