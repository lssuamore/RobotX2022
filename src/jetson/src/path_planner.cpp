...
#include "jetson/NED_acoustic.h"  // message type that holds position of beacon
...
//	STATES CONCERNED WITH STATE 1 OF  "DnD_path_planner"
...
// Wildlife Encounter and Avoid variables
...
void PATH_PLANNER_inspector()
{
	...
	if ((propulsion_system_topic_published) && (PP_state == 4))  // if the PP_propulsion_system_topic has been published and Wildlife Encounter and Avoid path planner
	{
...
// ACCEPTS: jetson::NED_acoustic from "A_source_location"
// RETURNS: (VOID)
//=============================================================================================================
void A_source_location_update(const jetson::NED_acoustic::ConstPtr& sourceloc) 
{
...
int main(int argc, char **argv)
{
	...
	// from acoustics
	//ros::Subscriber A_source_location_sub = nh10.subscribe("A_source_location", 1, A_source_location_update);  // acoustic pinger converted to NED
	...
	while(ros::ok())
	{
		...
		switch(PP_state)
		{
			...
			case 1:  // Dynamic navigation demonstration
				ROS_INFO("PATH_PLANNER: DnD State %i\n", DnD_path_planner);
				//  1 = Publish point 35 meters ahead
			...
			case 12:  // Figure eight wayfinding
				if (calculations_done)
				{
					if ((PP_USV_pose_update_state_msg.data) && (!propulsion_system_topic_published))  // if the USV pose updated but the propulsion_system_topic not published
					{
						if ((point == 0) || (point == 6))
						{
							position_tolerance = default_position_error_allowed*1.5;  // make the position tolerance larger
						}
						else
						{
						...
						if ((e_xy < position_tolerance) && (!E_reached))  //&& ((float)abs(e_psi) < e_psi_allowed) || (sign_change_check < 0)
						{
							point += 1;
				...
				{
					calculate_path();  // this function will generate the updated array of poses for the figure eight
