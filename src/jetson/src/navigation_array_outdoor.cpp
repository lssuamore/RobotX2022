...
int main(int argc, char **argv)
{
	...
	ros::Subscriber gpspos_sub = nh2.subscribe("/gps/fix", 10, GPS_Position_update);						// subscribes to GPS position
	ros::Subscriber imu_sub = nh3.subscribe("/wamv/sensors/imu/imu/data", 100, IMU_processor);										// subscribes to IMU
	ros::Subscriber gpsvel_sub = nh4.subscribe("/gps/fix_velocity", 100, GPS_Velocity_update);		// subscribes to GPS velocity
